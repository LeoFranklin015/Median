...
export function PasskeyAuth() {
  ...
  const [savedAccounts, setSavedAccounts] = useState<StoredSession[]>([]);
  const [showManualInput, setShowManualInput] = useState(false);
  const [showSendFunds, setShowSendFunds] = useState(false);

  // Load all saved accounts on mount
  useEffect(() => {
    const savedSessionsStr = localStorage.getItem('circle_sessions');
    if (savedSessionsStr) {
      try {
        const sessions: StoredSession[] = JSON.parse(savedSessionsStr);
        // Sort by most recently used
        sessions.sort((a, b) => b.lastUsed - a.lastUsed);
        setSavedAccounts(sessions);
      } catch (err) {
        console.error('Failed to load sessions:', err);
        localStorage.removeItem('circle_sessions');
      }
    }
  }, []);
  ...
  const saveSession = (session: StoredSession) => {
    ...
    // Remove existing session with same username
    sessions = sessions.filter(s => s.username !== session.username);
    ...
    localStorage.setItem('circle_sessions', JSON.stringify(sessions));
    setSavedAccounts(sessions);
  };

  const handleQuickSignIn = async (account: StoredSession) => {
    setIsLoading(true);
    setError(null);
    setUsername(account.username);

    try {
      // Use cached address - instant sign in
      setUser({
      ...
      const updatedSession = { ...account, lastUsed: Date.now() };
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      // Calculate smart account address (no deployment needed)
      const { smartAccount } = await createSmartAccountFromPasskey(credential);
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      if (existingAccount) {
        ...
        // Update last used timestamp
        const updatedSession = { ...existingAccount, lastUsed: Date.now() };
        saveSession(updatedSession);
        setIsLoading(false);
        return;
      }

      // Login with passkey (only if no cached session)
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="p-4 bg-gray-50 rounded-xl border border-gray-200">
          ...
          <ul className="space-y-2">
            ...
            <li className="flex items-center gap-2 text-sm text-gray-600">
              <span className="text-green-500">✓</span>
              <span>Account recovery built-in</span>
            </li>
          </ul>
        </div>
      </div>
